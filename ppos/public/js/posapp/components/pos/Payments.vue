    ...
    >
      ...
      <div class="overflow-y-auto px-2 pt-2" style="max-height: 75vh">
        ...
        <v-row class="px-1 py-0">
          ...
          <v-col
            cols="6"
            v-if="pos_profile.ppos_allow_sales_order && invoiceType == 'Order'"
          >
            <v-menu
            ...
            >
              <template v-slot:activator="{ props }">
                <v-text-field
                  v-model="invoice_doc.ppos_delivery_date"
              ...
              <v-date-picker
                v-model="delivery_date_internal"
                hide-header
                color="primary"
          ...
          <v-col cols="12" v-if="invoice_doc.ppos_delivery_date">
            <v-autocomplete
              clearable
              auto-select-first
              color="primary"
              ...
              <template v-slot:item="{ props, item }">
                <v-list-item v-bind="props">
                  <v-list-item-title class="text-primary subtitle-1">
                    {{ item.raw.address_title }}
                  </v-list-item-title>
                  <v-list-item-subtitle>
                    {{ item.raw.address_line1 }}
                  </v-list-item-subtitle>
                  <v-list-item-subtitle v-if="item.raw.address_line2">
          ...
          <v-col cols="12" v-if="pos_profile.ppos_display_additional_notes">
            <v-textarea
              class="pa-0"
              hide-details
              clearable
              ...
              v-model="invoice_doc.ppos_notes"
            ></v-textarea>
          </v-col>
        </v-row>

        <div v-if="pos_profile.ppos_allow_customer_purchase_order">
          <v-divider></v-divider>
          <v-row class="px-1 py-0" justify="center" align="start">
            ...
            <v-col cols="6">
              ...
              >
                ...
                <v-date-picker
                  v-model="po_date_internal"
                  hide-header
                  color="primary"
                  @update:model-value="onPoDateChange"
                >
                </v-date-picker>
              </v-menu>
            </v-col>
        ...
        <v-row class="px-1 py-0" align="start" no-gutters>
          <v-col
            cols="6"
            v-if="
              pos_profile.ppos_allow_write_off_change &&
              diff_payment > 0 &&
              !invoice_doc.is_return
            "
          >
          ...
          <v-col
            cols="6"
            v-if="pos_profile.ppos_allow_credit_sale && !invoice_doc.is_return"
          >
            <v-switch
    ...
    <div>
      <v-dialog v-model="phone_dialog" max-width="400px">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="error" @click="phone_dialog = false">{{
              __("Close")
            }}</v-btn>
            <v-btn color="primary" @click="request_payment">{{
              __("Request")
            }}</v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  data: () => ({
    ...
    date_menu: false,
    due_date_internal: null,
    po_date_menu: false,
    po_date_internal: null,
    delivery_date_internal: null,
    addresses: [],
    sales_persons: [],
    sales_person: "",
    paid_change: 0,
  ...
  methods: {
    onDeliveryDateChange(val) {
      if (val) {
        this.invoice_doc.ppos_delivery_date = new Date(val).toISOString().split('T')[0];
      }
      this.order_delivery_date = false;
    },
    onPoDateChange(val) {
      if (val) {
        this.invoice_doc.po_date = new Date(val).toISOString().split('T')[0];
      }
      this.po_date_menu = false;
    },
    onDueDateChange(val) {
      if (val) {
        this.invoice_doc.due_date = new Date(val).toISOString().split('T')[0];
      }
    ...
    submit(event, payment_received = false, print = false) {
      ...
      if (
        !this.pos_profile.ppos_allow_partial_payment &&
        this.total_payments <
      ...
      if (
        this.pos_profile.ppos_allow_partial_payment &&
        !this.pos_profile.ppos_allow_credit_sale &&
    ...
    submit_invoice(print) {
      ...
      const vm = this;
      frappe.call({
        method: "ppos.ppos.api.posapp.submit_invoice",
        args: {
          data: data,
    ...
    get_available_credit(e) {
      ...
      if (e) {
        frappe
          .call("ppos.ppos.api.posapp.get_available_credit", {
    ...
    get_addresses() {
      ...
      frappe.call({
        method: "ppos.ppos.api.posapp.get_customer_addresses",
    ...
    get_sales_person_names() {
      ...
      frappe.call({
        method: "ppos.ppos.api.posapp.get_sales_person_names",
        callback: function (r) {
          if (r.message) {
    ...
    request_payment() {
      ...
      frappe
        ...
        .then(() => {
          frappe
            .call({
              method: "ppos.ppos.api.posapp.create_payment_request",
              args: {
                doc: vm.invoice_doc,
              },
            })
    ...
    get_mpesa_modes() {
      const vm = this;
      frappe.call({
        method: "ppos.ppos.api.m_pesa.get_mpesa_mode_of_payment",
  ...
  computed: {
    total_payments() {
      let total = 0;
      if (this.invoice_doc) {
        total += parseFloat(this.invoice_doc.loyalty_amount || 0);
    ...
    vaildatPayment() {
      if (this.pos_profile.ppos_allow_sales_order) {
        if (
          this.invoiceType == "Order" &&
          (!this.invoice_doc || !this.invoice_doc.ppos_delivery_date)
        ) {
          return true;
        } else {
          return false;
    ...
    this.$nextTick(function () {
      ...
      evntBus.on("update_invoice_type", (data) => {
        ...
        if (this.invoice_doc && data != "Order") {
          this.invoice_doc.ppos_delivery_date = null;
          this.invoice_doc.ppos_notes = null;
