<template>
  <v-row justify="center">
    ...
    >
      <v-card>
        <v-card-title>
          <span v-if="customer_id" class="headline text-primary">{{
            __('Update Customer')
          }}</span>
          <span v-else class="headline text-primary">{{
            __('Create Customer')
          }}</span>
        </v-card-title>
...
export default {
  ...
  watch: {
    birthday(val) {
      if (val) {
        this.birthday_date = new Date(val);
      } else {
  ...
  methods: {
    onBirthdayChange(val) {
      if (val) {
        ...
        this.birthday = d.toISOString().split('T')[0];
      } else {
        this.birthday = null;
      }
    ...
    submit_dialog() {
      ...
      if (this.customer_name) {
        ...
        frappe.call({
          method: 'ppos.ppos.api.posapp.create_customer',
          args: args,
          callback: (r) => {
