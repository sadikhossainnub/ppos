  ...
  <div>
    ...
    >
      <v-row align="center" class="items px-2 py-1">
        <v-col
          v-if="pos_profile.ppos_allow_sales_order"
          cols="9"
          class="pb-2 pr-0"
        >
          <Customer></Customer>
        ...
        <v-col v-if="pos_profile.ppos_allow_sales_order" cols="3" class="pb-2">
          <v-select
            density="compact"
            hide-details
            variant="outlined"
      ...
      <v-row
        ...
        v-if="pos_profile.ppos_use_delivery_charges"
      >
      ...
      <v-row
        align="center"
        class="items px-2 py-1 mt-0 pt-0"
        v-if="pos_profile.ppos_allow_change_posting_date"
      >
        <v-col
          v-if="pos_profile.ppos_allow_change_posting_date"
          cols="4"
      ...
      <div class="my-0 py-0 overflow-y-auto" style="max-height: 65vh">
        <template @mouseover="style = 'cursor: pointer'">
          ...
          >
            ...
            >
            <template v-slot:item.ppos_is_offer="{ item }">
              <v-checkbox-btn
                :model-value="!!item.ppos_is_offer || !!item.ppos_is_replace"
                disabled
              ></v-checkbox-btn>
            </template>
            ...
            <template v-slot:expanded-row="{ columns, item }">
              <td :colspan="columns.length" class="ma-0 pa-0">
                <v-row class="ma-0 pa-0">
                  <v-col cols="1">
                    <v-btn
                      :disabled="!!item.ppos_is_offer || !!item.ppos_is_replace"
                      icon
                      color="error"
                      variant="text"
                ...
                <v-row class="ma-0 pa-0">
                  ...
                  <v-col cols="4">
                    <v-text-field
                      ...
                      :disabled="
                        !!item.ppos_is_offer ||
                        !!item.ppos_is_replace ||
                        !!item.ppos_offer_applied ||
                        !pos_profile.ppos_allow_user_to_edit_rate ||
                        !!invoice_doc.is_return
                          ? true
                          : false
                      "
                  ...
                  <v-col cols="4">
                    <v-text-field
                      ...
                      :disabled="
                        ...
                        !!item.ppos_is_replace ||
                        item.ppos_offer_applied ||
                        !pos_profile.ppos_allow_user_to_edit_item_discount ||
                        !!invoice_doc.is_return
                          ? true
                          : false
                      "
                  ...
                  <v-col align="center" cols="4" v-if="item.ppos_offer_applied">
                    <v-checkbox
                      density="compact"
                      :label="frappe._('Offer Applied')"
                      v-model="item.ppos_offer_applied"
                      readonly
                      hide-details
                  ...
                  <v-col
                    cols="4"
                    v-if="
                      pos_profile.ppos_allow_sales_order &&
                      invoiceType == 'Order'
                    "
                  >
                    <v-menu
                      ref="item_delivery_date"
                      v-model="item.item_delivery_date_menu"
                      :close-on-content-click="false"
                      transition="scale-transition"
                    >
                      <template v-slot:activator="{ props }">
                        <v-text-field
                          v-model="item.ppos_delivery_date"
                      ...
                      >
                        ...
                        <v-btn
                          variant="text"
                          color="primary"
                          @click="item.item_delivery_date_menu = false"
                        >
                          Cancel
                        </v-btn>
                        <v-btn
                          ...
                          @click="
                            [
                              item.item_delivery_date_menu = false,
                  ...
                  <v-col
                    cols="8"
                    v-if="pos_profile.ppos_display_additional_notes"
                  >
                    <v-textarea
                      class="pa-0"
                      variant="outlined"
      ...
      <v-row no-gutters>
        <v-col cols="7">
          <v-row no-gutters class="pa-1 pt-9 pr-1">
            ...
            <v-col
              v-if="!pos_profile.ppos_use_percentage_discount"
              cols="6"
              class="pa-1"
            >
              <v-text-field
                ...
                :prefix="currencySymbol(pos_profile.currency)"
                :disabled="
                  !pos_profile.ppos_allow_user_to_edit_additional_discount ||
          ...
          <v-row no-gutters class="pa-1 pt-2 pl-0">
            ...
            <v-col cols="6" class="pa-1">
              <v-btn
                block
                class="pa-0"
                :class="{ 'disable-events': !pos_profile.ppos_allow_return }"
                color="secondary"
                @click="open_returns"
                >{{ __("Return") }}</v-btn
              >
            ...
            <v-col
              v-if="pos_profile.ppos_allow_print_draft_invoices"
              cols="6"
              class="pa-1"
            >
              <v-btn
  ...
  data() {
    return {
      ...
      items_headers: [
        ...
        { title: __("is Offer"), key: "ppos_is_offer", align: "center" },
      ],
    };
  },
  ...
  methods: {
    remove_item(item) {
      const index = this.items.findIndex(
        (el) => el.ppos_row_id == item.ppos_row_id
      );
      if (index >= 0) {
    ...
    get_new_item(item) {
      ...
      if (!item.ppos_is_offer) {
        item.ppos_is_offer = 0;
      }
      if (!item.ppos_is_replace) {
        item.ppos_is_replace = "";
      }
      ...
      new_item.ppos_offers = JSON.stringify([]);
      new_item.ppos_offer_applied = 0;
      new_item.ppos_is_offer = item.ppos_is_offer;
      new_item.ppos_is_replace = item.ppos_is_replace || null;
      new_item.is_free_item = 0;
      new_item.ppos_notes = "";
      new_item.ppos_delivery_date = "";
      new_item.ppos_row_id = this.makeid(20);
      if (
        (!this.pos_profile.ppos_auto_set_batch && new_item.has_batch_no) ||
    ...
    cancel_invoice() {
      ...
      this.invoiceType = this.pos_profile.ppos_default_sales_order
        ? "Order"
        : "Invoice";
      this.invoiceTypes = ["Invoice", "Order"];
      this.posting_date = frappe.datetime.nowdate();
      if (doc.name && this.pos_profile.ppos_allow_delete) {
        frappe.call({
          method: "ppos.ppos.api.posapp.delete_invoice",
    ...
    new_invoice(data = {}) {
      ...
      if (!data.name && !data.is_return) {
        ...
        this.ppos_offers = data.ppos_offers || [];
        this.items.forEach((item) => {
          if (!item.ppos_row_id) {
            item.ppos_row_id = this.makeid(20);
          }
          if (item.batch_no) {
    ...
    get_invoice_doc() {
      ...
      doc.ppos_pos_opening_shift = this.pos_opening_shift.name;
      doc.payments = this.get_payments();
      doc.taxes = [];
      doc.is_return = this.invoice_doc.is_return;
      doc.return_against = this.invoice_doc.return_against;
      doc.ppos_offers = this.ppos_offers;
      doc.ppos_coupons = this.ppos_coupons;
      doc.ppos_delivery_charges = this.selcted_delivery_charges.name;
      doc.ppos_delivery_charges_rate = this.delivery_charges_rate || 0;
    ...
    async get_invoice_from_order_doc() {
      ...
      doc.items.forEach((item) => {
        ...
        if (updatedData) {
          ...
          item.ppos_row_id = updatedData.ppos_row_id;
          item.ppos_offers = updatedData.ppos_offers;
          item.ppos_offer_applied = updatedData.ppos_offer_applied;
          item.ppos_is_offer = updatedData.ppos_is_offer;
          item.ppos_is_replace = updatedData.ppos_is_replace;
          ...
          item.ppos_notes = updatedData.ppos_notes;
          item.ppos_delivery_date = updatedData.ppos_delivery_date;
    ...
    get_invoice_items() {
      ...
      this.items.forEach((item) => {
        const new_item = {
          ...
          ppos_row_id: item.ppos_row_id,
          ppos_offers: item.ppos_offers,
          ppos_offer_applied: item.ppos_offer_applied,
          ppos_is_offer: item.ppos_is_offer,
          ppos_is_replace: item.ppos_is_replace,
          ...
          ppos_notes: item.ppos_notes,
          ppos_delivery_date: item.ppos_delivery_date,
    ...
    update_invoice(doc) {
      const vm = this;
      frappe.call({
        method: "ppos.ppos.api.posapp.update_invoice",
        args: {
          data: doc,
        },
        async: false,
    ...
    update_invoice_from_order(doc) {
      const vm = this;
      frappe.call({
        method: "ppos.ppos.api.posapp.update_invoice_from_order",
        args: {
          data: doc,
        },
        async: false,
    ...
    async show_payment() {
      ...
      if (this.invoice_doc.doctype == "Sales Order") {
        ...
        frappe.call({
          method:
            "ppos.ppos.api.posapp.get_sales_invoice_child_table",
          args: {
    ...
    validate() {
      ...
      this.items.forEach((item) => {
        if (
          this.pos_profile.ppos_max_discount_allowed &&
          !item.ppos_offer_applied
        ) {
          if (item.discount_amount && this.flt(item.discount_amount) > 0) {
            ...
            if (
              discount_percentage > this.pos_profile.ppos_max_discount_allowed
            ) {
              evntBus.$emit("show_mesage", {
                text: __(
                  `Discount percentage for item '{0}' cannot be greater than {1}%`,
                  [item.item_name, this.pos_profile.ppos_max_discount_allowed]
                ),
                color: "error",
              });
              value = false;
        ...
        if (this.pos_profile.ppos_allow_user_to_edit_additional_discount) {
          const clac_percentage = (this.discount_amount / this.Total) * 100;
          if (clac_percentage > this.pos_profile.ppos_max_discount_allowed) {
            evntBus.$emit("show_mesage", {
    ...
    get_draft_invoices() {
      const vm = this;
      frappe.call({
        method: "ppos.ppos.api.posapp.get_draft_invoices",
        args: {
          ...
          if (r.message) {
            items.forEach((item) => {
              const updated_item = r.message.find(
                (element) => element.ppos_row_id == item.ppos_row_id
              );
    ...
    update_item_detail(item) {
      ...
      const vm = this;
      frappe.call({
        method: "ppos.ppos.api.posapp.get_item_detail",
        args: {
        ...
        callback: function (r) {
          if (r.message) {
            ...
            if (
              item.has_batch_no &&
              vm.pos_profile.ppos_auto_set_batch &&
              !item.batch_no &&
              data.batch_no_data
            ) {
              item.batch_no_data = data.batch_no_data;
            ...
            if (data.has_pricing_rule) {
              ...
              vm.pos_profile.ppos_apply_customer_discount &&
              vm.customer_info.ppos_discount > 0 &&
              vm.customer_info.ppos_discount <= 100
            ) {
              if (
                item.ppos_is_offer == 0 &&
                !item.ppos_is_replace &&
                item.ppos_offer_applied == 0
              ) {
                if (item.max_discount > 0) {
                  item.discount_percentage =
                    item.max_discount < vm.customer_info.ppos_discount
                      ? item.max_discount
                      : vm.customer_info.ppos_discount;
                } else {
                  item.discount_percentage = vm.customer_info.ppos_discount;
                }
              }
            }
            if (!item.batch_price) {
    ...
    fetch_customer_details() {
      ...
      if (this.customer) {
        frappe.call({
          method: "ppos.ppos.api.posapp.get_customer_info",
          args: {
    ...
    set_batch_qty(item, value, update = true) {
      const existing_items = this.items.filter(
        (element) =>
          ...
          element.ppos_row_id != item.ppos_row_id
      );
      const used_batches = {};
    ...
    checkOfferIsAppley(item, offer) {
      let applied = false;
      const item_offers = JSON.parse(item.ppos_offers);
      for (const row_id of item_offers) {
        const exist_offer = this.ppos_offers.find((el) => row_id == el.row_id);
    ...
    getItemFromRowID(row_id) {
      const item = this.items.find((el) => el.ppos_row_id == row_id);
      return item;
    },
    ...
    checkOfferCoupon(offer) {
      if (offer.coupon_based) {
        const coupon = this.ppos_coupons.find(
    ...
    getItemOffer(offer) {
      ...
      if (offer.apply_on === "Item Code") {
        if (this.checkOfferCoupon(offer)) {
          this.items.forEach((item) => {
            if (!item.ppos_is_offer && item.item_code === offer.item) {
              const items = [];
              if (
    ...
    getGroupOffer(offer) {
      ...
      if (offer.apply_on === "Item Group") {
        if (this.checkOfferCoupon(offer)) {
          ...
          let total_amount = 0;
          this.items.forEach((item) => {
            if (!item.ppos_is_offer && item.item_group === offer.item_group) {
              if (
                offer.offer === "Item Price" &&
                item.ppos_offer_applied &&
                !this.checkOfferIsAppley(item, offer)
    ...
    getBrandOffer(offer) {
      ...
      if (offer.apply_on === "Brand") {
        if (this.checkOfferCoupon(offer)) {
          ...
          let total_amount = 0;
          this.items.forEach((item) => {
            if (!item.ppos_is_offer && item.brand === offer.brand) {
              if (
                offer.offer === "Item Price" &&
                item.ppos_offer_applied &&
                !this.checkOfferIsAppley(item, offer)
    ...
    getTransactionOffer(offer) {
      ...
      if (offer.apply_on === "Transaction") {
        if (this.checkOfferCoupon(offer)) {
          let total_qty = 0;
          this.items.forEach((item) => {
            if (!item.ppos_is_offer && !item.ppos_is_replace) {
    ...
    updateInvoiceOffers(offers) {
      this.ppos_offers.forEach((invoiceOffer) => {
      ...
      offers.forEach((offer) => {
        const existOffer = this.ppos_offers.find(
        ...
        if (existOffer) {
          ...
          if (
            ...
            const item_to_remove = this.items.find(
              (item) => item.ppos_row_id == existOffer.give_item_row_id
            );
            if (item_to_remove) {
              const updated_item_offers = offer.items.filter(
                (row_id) => row_id != item_to_remove.ppos_row_id
              );
            ...
            if (offer.replace_cheapest_item) {
              ...
              const oldBaseItem = this.items.find(
                (el) => el.ppos_row_id == item_to_remove.ppos_is_replace
              );
              newItemOffer.qty = item_to_remove.qty;
              if (oldBaseItem && !oldBaseItem.ppos_is_replace) {
                ...
                restoredItem.ppos_is_offer = 0;
                this.items.unshift(restoredItem);
              }
              newItemOffer.ppos_is_offer = 0;
              newItemOffer.ppos_is_replace = cheapestItem.ppos_row_id;
              const diffQty = cheapestItem.qty - newItemOffer.qty;
              if (diffQty <= 0) {
                newItemOffer.qty += diffQty;
                this.remove_item(cheapestItem);
                newItemOffer.ppos_row_id = cheapestItem.ppos_row_id;
                newItemOffer.ppos_is_replace = newItemOffer.ppos_row_id;
              } else {
                cheapestItem.qty = diffQty;
              }
            }
            this.items.unshift(newItemOffer);
            existOffer.give_item_row_id = newItemOffer.ppos_row_id;
              ...
              if (diff > 0) {
                ...
                const existItem = itemsList.find(
                  (el) =>
                    ...
                    el.ppos_is_replace != offerItem.ppos_row_id
                );
                if (existItem) {
    ...
    removeApplyOffer(invoiceOffer) {
      if (invoiceOffer.offer === "Item Price") {
        ...
        const index = this.ppos_offers.findIndex(
          (el) => el.row_id === invoiceOffer.row_id
        );
        this.ppos_offers.splice(index, 1);
      }
      if (invoiceOffer.offer === "Give Product") {
        const item_to_remove = this.items.find(
          (item) => item.ppos_row_id == invoiceOffer.give_item_row_id
        );
        const index = this.ppos_offers.findIndex(
          (el) => el.row_id === invoiceOffer.row_id
        );
    ...
    applyNewOffer(offer) {
      ...
      if (offer.offer === "Give Product") {
        ...
        if (
          ...
          item.ppos_is_replace = itemsRowID[0];
          const baseItem = this.items.find(
            (el) => el.ppos_row_id == item.ppos_is_replace
          );
          ...
          if (diffQty <= 0) {
            ...
            item.ppos_row_id = item.ppos_is_replace;
          } else {
            baseItem.qty = diffQty;
          }
          this.items.unshift(item);
          offer.give_item_row_id = item.ppos_row_id;
        } else if (
          offer.apply_on == "Item Group" &&
          offer.apply_type == "Item Group" &&
          offer.replace_cheapest_item
          ...
          item.ppos_is_offer = 0;
          item.ppos_is_replace = baseItem.ppos_row_id;
          const diffQty = baseItem.qty - offer.given_qty;
          if (diffQty <= 0) {
            item.qty = baseItem.qty;
            this.remove_item(baseItem);
      ...
      this.ppos_offers.push(newOffer);
    ...
    ApplyOnGiveProduct(offer, item_code) {
      ...
      new_item.ppos_offers = JSON.stringify([]);
      new_item.ppos_offer_applied = 0;
      new_item.ppos_is_offer = 1;
      new_item.ppos_is_replace = null;
      new_item.ppos_notes = "";
      new_item.ppos_delivery_date = "";
    ...
    ApplyOnPrice(offer) {
      this.items.forEach((item) => {
        if (offer.items.includes(item.ppos_row_id)) {
          const item_offers = JSON.parse(item.ppos_offers);
    ...
    addOfferToItems(offer) {
      ...
      offer_items.forEach((el) => {
        this.items.forEach((exist_item) => {
          if (exist_item.ppos_row_id == el) {
            const item_offers = JSON.parse(exist_item.ppos_offers);
            if (!item_offers.includes(offer.row_id)) {
              item_offers.push(offer.row_id);
              if (offer.offer === "Item Price") {
                exist_item.ppos_offer_applied = 1;
              }
            }
            exist_item.ppos_offers = JSON.stringify(item_offers);
          }
        });
      });
    },
      ...
      offer_items.forEach((el) => {
        this.items.forEach((exist_item) => {
          if (exist_item.ppos_row_id == el) {
            ...
            if (offer.offer === "Item Price") {
              exist_item.ppos_offer_applied = 0;
            }
            exist_item.ppos_offers = JSON.stringify(updated_item_offers);
          }
        });
      });
    },
    ...
    validate_due_date(item) {
      ...
      const new_date = Date.parse(item.ppos_delivery_date);
      if (new_date < parse_today) {
        setTimeout(() => {
          item.ppos_delivery_date = today;
        }, 0);
      }
    },
    ...
    print_draft_invoice() {
      if (!this.pos_profile.ppos_allow_print_draft_invoices) {
        evntBus.emit("show_mesage", {
    ...
    set_delivery_charges() {
      ...
      if (
        !this.pos_profile ||
        !this.customer ||
        !this.pos_profile.ppos_use_delivery_charges
      ) {
      ...
      frappe.call({
        method:
          "ppos.ppos.api.posapp.get_applicable_delivery_charges",
        args: {
          company: this.pos_profile.company,
          pos_profile: this.pos_profile.name,
          customer: this.customer,
  ...
  mounted() {
    ...
    evntBus.on("load_invoice", (data) => {
      ...
      if (this.invoice_doc.is_return) {
        ...
        evntBus.emit("set_pos_coupons", data.ppos_coupons);
      }
    });
  ...
  watch: {
    ...
    discount_amount() {
      if (!this.discount_amount || this.discount_amount == 0) {
        this.additional_discount_percentage = 0;
      } else if (this.pos_profile.ppos_use_percentage_discount) {
        this.additional_discount_percentage =
