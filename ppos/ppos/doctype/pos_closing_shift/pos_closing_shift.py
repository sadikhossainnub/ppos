...
class POSClosingShift(Document):
    ...
    def delete_draft_invoices(self):
        if frappe.get_value("POS Profile", self.pos_profile, "ppos_allow_delete"):
            data = frappe.db.sql(
                """
                select
                    name
                from
                    `tabSales Invoice`
                where
                    docstatus = 0 and ppos_is_printed = 0 and ppos_pos_opening_shift = %s
                """,
    ...
    def get_payment_reconciliation_details(self):
        ...
        return frappe.render_template(
            "ppos/ppos/doctype/pos_closing_shift/closing_shift_details.html",
...
def get_pos_invoices(pos_opening_shift):
	...
	from
		`tabSales Invoice`
	where
		docstatus = 1 and ppos_pos_opening_shift = %s
	""",
        (pos_opening_shift),
        as_dict=1,
    )
