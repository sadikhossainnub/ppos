...
frappe.ui.form.on('POS Closing Shift', {
	onload: function (frm) {
		...
		frm.set_query("user", function (doc) {
			return {
				query: "ppos.ppos.doctype.pos_closing_shift.pos_closing_shift.get_cashiers",
	...
	get_pos_invoices (frm) {
		frappe.call({
			method: 'ppos.ppos.doctype.pos_closing_shift.pos_closing_shift.get_pos_invoices',
			args: {
	...
	get_pos_payments (frm) {
		frappe.call({
			method: 'ppos.ppos.doctype.pos_closing_shift.pos_closing_shift.get_payments_entries',
			args: {
				pos_opening_shift: frm.doc.pos_opening_shift,
			},
			callback: (r) => {
...
function add_to_payments (d, frm) {
	d.payments.forEach(p => {
		...
		if (payment) {
			let amount = p.amount;
			let cash_mode_of_payment = get_value("POS Profile", frm.doc.pos_profile, 'ppos_cash_mode_of_payment');
