...
from ppos.ppos.api.m_pesa import submit_mpesa_payment
...
def process_pos_payment(payload):
    ...
    if not data.pos_profile.get("ppos_use_ppos_payments"):
        frappe.throw(_("PPOS Payments is not enabled for this POS Profile"))
    ...
    allow_make_new_payments = data.pos_profile.get("ppos_allow_make_new_payments")
    allow_reconcile_payments = data.pos_profile.get("ppos_allow_reconcile_payments")
    allow_mpesa_reconcile_payments = data.pos_profile.get(
        "ppos_allow_mpesa_reconcile_payments"
    )
    today = nowdate()
