...
from ppos.ppos.doctype.pos_coupon.pos_coupon import update_coupon_code_count
from ppos.ppos.api.posapp import get_company_domain
...
def add_loyalty_point(invoice_doc):
    for offer in invoice_doc.ppos_offers:
...
def create_sales_order(doc):
    if (
        doc.ppos_pos_opening_shift
        and doc.pos_profile
        and doc.is_pos
        and doc.ppos_delivery_date
        and not doc.update_stock
        and frappe.get_value("POS Profile", doc.pos_profile, "ppos_allow_sales_order")
    ):
        sales_order_doc = make_sales_order(doc.name)
        if sales_order_doc:
            sales_order_doc.ppos_notes = doc.ppos_notes
...
def make_sales_order(source_name, target_doc=None, ignore_permissions=True):
    ...
    def update_item(obj, target, source_parent):
        ...
        target.delivery_date = (
            obj.ppos_delivery_date or source_parent.ppos_delivery_date
        )
        ...
        source_name,
        {
            ...
            "Sales Invoice Item": {
                ...
                "field_map": {
                    ...
                    "Warehouse": "warehouse",
                    "delivery_date": "ppos_delivery_date",
                    "ppos_notes": "ppos_notes",
                },
...
def update_coupon(doc, transaction_type):
    for coupon in doc.ppos_coupons:
...
def auto_set_delivery_charges(doc):
    ...
    if not frappe.get_cached_value(
        "POS Profile", doc.pos_profile, "ppos_auto_set_delivery_charges"
    ):
        return
    ...
    delivery_charges = get_applicable_delivery_charges(
        ...
        doc.ppos_delivery_charges,
        restrict=True,
    )

    if doc.ppos_delivery_charges:
        if doc.ppos_delivery_charges_rate:
            return
        else:
            if len(delivery_charges) > 0:
                doc.ppos_delivery_charges_rate = delivery_charges[0].rate
    else:
        if len(delivery_charges) > 0:
            doc.ppos_delivery_charges = delivery_charges[0].name
            doc.ppos_delivery_charges_rate = delivery_charges[0].rate
        else:
            doc.ppos_delivery_charges = None
            doc.ppos_delivery_charges_rate = None
...
def calc_delivery_charges(doc):
    ...
    if not doc.is_new():
        ...
        if not doc.ppos_delivery_charges and not old_doc.ppos_delivery_charges:
            return
    else:
        if not doc.ppos_delivery_charges:
            return
    if not doc.ppos_delivery_charges:
        doc.ppos_delivery_charges_rate = 0

    charges_doc = None
    if doc.ppos_delivery_charges:
        charges_doc = frappe.get_cached_doc(
            "Delivery Charges", doc.ppos_delivery_charges
        )
        doc.ppos_delivery_charges_rate = charges_doc.default_rate
        charges_profile = next(
            (i for i in charges_doc.profiles if i.pos_profile == doc.pos_profile), None
        )
        if charges_profile:
            doc.ppos_delivery_charges_rate = charges_profile.rate

    if old_doc and old_doc.ppos_delivery_charges:
        old_charges = next(
            (
                i
                for i in doc.taxes
                if i.charge_type == "Actual"
                and i.description == old_doc.ppos_delivery_charges
            ),
            None,
        )
        if old_charges:
    ...
    if doc.ppos_delivery_charges:
        doc.append(
            ...
            {
                "charge_type": "Actual",
                "description": doc.ppos_delivery_charges,
                "tax_amount": doc.ppos_delivery_charges_rate,
...
def validate_shift(doc):
    if doc.ppos_pos_opening_shift and doc.pos_profile and doc.is_pos:
        # check if shift is open
        shift = frappe.get_cached_doc("POS Opening Shift", doc.ppos_pos_opening_shift)
