...
from ppos.ppos.doctype.pos_coupon.pos_coupon import check_coupon_code
from ppos.ppos.doctype.delivery_charges.delivery_charges import (
...
def get_items(
    ...
    ttl = _pos_profile.get("ppos_server_cache_duration")
    if ttl:
        ttl = int(ttl) * 30
    ...
    def _get_items(pos_profile, price_list, item_group, search_value, customer=None):
        ...
        today = nowdate()
        data = dict()
        ppos_display_items_in_stock = pos_profile.get("ppos_display_items_in_stock")
        search_serial_no = pos_profile.get("ppos_search_serial_no")
        search_batch_no = pos_profile.get("ppos_search_batch_no")
        ppos_show_template_items = pos_profile.get("ppos_show_template_items")
        ...
        if use_limit_search:
            search_limit = pos_profile.get("ppos_search_limit") or 500
            if search_value:
        ...
        if not ppos_show_template_items:
        ...
        if items_data:
            ...
            for item in items_data:
                                    ...
                                    batch_no_data.append(
                                        {
                                            ...
                                            "batch_price": batch_doc.ppos_batch_price,
                ...
                item_stock_qty = 0
                if pos_profile.get("ppos_display_items_in_stock") or use_limit_search:
                    item_stock_qty = get_stock_availability(
                        item_code, pos_profile.get("warehouse")
                    )
                attributes = ""
                if pos_profile.get("ppos_show_template_items") and item.has_variants:
                    attributes = get_item_attributes(item.item_code)
                item_attributes = ""
                if pos_profile.get("ppos_show_template_items") and item.variant_of:
                    item_attributes = frappe.get_all(
                        "Item Variant Attribute",
                        fields=["attribute", "attribute_value"],
                        filters={"parent": item.item_code, "parentfield": "attributes"},
                    )
                if ppos_display_items_in_stock and (
    ...
    if _pos_profile.get("ppos_use_server_cache"):
...
def update_invoice(data):
    ...
    allow_zero_rated_items = frappe.get_cached_value(
        "POS Profile", invoice_doc.pos_profile, "ppos_allow_zero_rated_items"
    )
    ...
    if frappe.get_cached_value(
        "POS Profile", invoice_doc.pos_profile, "ppos_tax_inclusive"
    ):
...
def submit_invoice(invoice, data):
    ...
    if invoice.get("ppos_delivery_date"):
        invoice_doc.update_stock = 0
    mop_cash_list = [
        i.mode_of_payment
    ...
    if frappe.get_value("POS Profile", invoice_doc.pos_profile, "ppos_auto_set_batch"):
        invoice_doc.set_batch_nos()
    set_batch_nos_for_bundels(invoice_doc, "warehouse", throw=True)

    invoice_doc.flags.ignore_permissions = True
    frappe.flags.ignore_account_permission = True
    invoice_doc.ppos_is_printed = 1
    invoice_doc.save()
    ...
    if frappe.get_value(
        ...
        "ppos_allow_submissions_in_background_job",
    ):
        invoices_list = frappe.get_all(
            "Sales Invoice",
            filters={
                "ppos_pos_opening_shift": invoice_doc.ppos_pos_opening_shift,
                "docstatus": 0,
                "ppos_is_printed": 1,
            },
        )
            ...
            payment_entry_doc = frappe.get_doc(
                {
                    ...
                    "reference_no": invoice_doc.ppos_pos_opening_shift,
...
def get_draft_invoices(pos_opening_shift):
    invoices_list = frappe.get_list(
        "Sales Invoice",
        filters={
            "ppos_pos_opening_shift": pos_opening_shift,
            "docstatus": 0,
            "ppos_is_printed": 0,
        },
        fields=["name"],
...
def delete_invoice(invoice):
    if frappe.get_value("Sales Invoice", invoice, "ppos_is_printed"):
...
def create_customer(
    ...
    if method == "create":
        ...
        if pos_profile.get("ppos_allow_duplicate_customer_names") or not is_exist:
            customer = frappe.get_doc(
                {
                    "doctype": "Customer",
                    "customer_name": customer_name,
                    "ppos_referral_company": company,
                    "tax_id": tax_id,
                    "mobile_no": mobile_no,
                    "email_id": email_id,
                    "ppos_referral_code": referral_code,
                    "ppos_birthday": birthday,
        ...
        customer_doc.ppos_referral_company = company
        customer_doc.tax_id = tax_id
        customer_doc.ppos_referral_code = referral_code
        customer_doc.ppos_birthday = birthday
...
def get_items_from_barcode(selling_price_list, currency, barcode):
    search_item = frappe.get_all(
        ...
        fields=["parent", "barcode", "ppos_uom"],
    )
...
def get_version():
    try:
        import erpnext
        return int(erpnext.__version__.split('.')[0])
    except Exception:
...
def get_customer_info(customer):
    ...
    res["birthday"] = customer.ppos_birthday
    res["gender"] = customer.gender
    res["tax_id"] = customer.tax_id
    res["ppos_discount"] = customer.ppos_discount
